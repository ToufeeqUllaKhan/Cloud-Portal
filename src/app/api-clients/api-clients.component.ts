...
@Component({
  ...
  styleUrls: ['./api-clients.component.css']
})
export class ApiClientsComponent implements OnInit {
  ...
  data = []; arr = [];
  in: number = 0; activeRegionsList = [];
  isNextVisible: Boolean = false;
  filterClients = []; clientArrList = [];
  clientsData = []; RolelevelProjects = []; apiMenuList: any;
    ...
    $(document).ready(function () {
      ...
      $(window).resize(function () {
        ...
        if (browserZoomLevel == 110) {
          $('.col-lg-3').css('max-width', '22%');
          $('.h4-title').css('width', '153px');
        }
        if (browserZoomLevel == 125) {
          $('.col-lg-3').css('max-width', '25%');
          $('.h4-title').css('width', '146px');
        }
